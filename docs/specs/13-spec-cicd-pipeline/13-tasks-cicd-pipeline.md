...
### [x] 2.0 Build & Publish Deployable Image to GHCR (`cd.yml` build job)
...
- [x] 2.1 Create `.github/workflows/cd.yml` triggered on `push` to `main` (add `workflow_dispatch`
  for manual test runs), with a `build` job and `permissions: contents: read, packages: write` and a
  `timeout-minutes` guard.
- [x] 2.2 Add build steps: `actions/checkout@v4`; `actions/setup-java@v4` (Temurin 17, `cache:
  maven`); compute tags from `${{ github.sha }}` and `latest`.
- [x] 2.3 Log in to GHCR with `docker/login-action@v3` using `${{ github.actor }}` and
  `${{ secrets.GITHUB_TOKEN }}`.
- [x] 2.4 Build the image with `./mvnw spring-boot:build-image` named
  `ghcr.io/${{ github.repository }}:${{ github.sha }}`, then `docker tag` it `:latest` and
  `docker push` both tags.
- [x] 2.5 Expose the published `:<sha>` reference as a `build` job output (for the future deploy
  job) using `$GITHUB_OUTPUT`.
- [x] 2.6 Validate by running the workflow (via `workflow_dispatch` or a merge to `main`); capture
